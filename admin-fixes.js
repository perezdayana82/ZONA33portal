// ZONA 33 · Admin final fix: Clientes + Reservas
(function(){
  ...
  window.go=function(page){
    if(page===ADMIN_RESERVATIONS && typeof role!=='undefined' && role==='admin'){
      ...
      if(content) return window.adminReservations(content);
    }
    return baseGo(page);
  };
  ...
  window.adminClients=function(c){
    ...
    const draw=(rows)=>{
      document.querySelector('#adminClientBody').innerHTML=rows.map(x=>`<tr><td><b>${esc(x.full_name||'Sin nombre')}</b></td><td>${esc(x.email||'—')}</td><td>${esc(x.phone||'—')}</td><td><span class="pill ${x.is_active?'ok':'bad'}">${x.is_active?'ACTIVO':'INACTIVO'}</span></td><td><div class="actions"><button class="btn danger" onclick="deleteClient('${x.id}',${JSON.stringify(x.full_name||'este cliente')})">ELIMINAR</button></div></td></tr>`).join('') || '<tr><td colspan="5">No hay clientes.</td></tr>';
    };
    draw(clients);
