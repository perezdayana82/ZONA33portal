...
const loadCoachFix=()=>{if(document.querySelector('script[data-z33-coach-fix]'))return;const s=document.createElement('script');s.src='/enhancements8.js?v=20260827-coach1';s.defer=true;s.dataset.z33CoachFix='1';document.body.appendChild(s)};
const fix=()=>{document.querySelectorAll('.brand img').forEach(img=>{img.src=logo;img.alt='ZONA 33';img.removeAttribute('onerror')});document.querySelectorAll('button').forEach(btn=>{if(String(btn.textContent||'').trim().toUpperCase()==='SITIO')btn.onclick=()=>window.location.assign(LANDING)});document.querySelectorAll('a.brand').forEach(a=>{a.href=LANDING});loadCMS();loadSchedule();loadAdminTools();loadCoachFix()};
